...
syn region razorInnerHTMLTag    start=/<\/\@!/ end=/>/ contains=htmlTagN,htmlString,htmlArg,htmlValue,htmlTagError,htmlEvent,htmlCssDefinition,@htmlPreproc,@htmlArgCluster display contained fold
syn region razorInnerHTMLEndTag start=/<\//    end=/>/ contains=htmlTagN,htmlTagError display contained

syn region razorInnerHTML start=/\_^\s*\zs<\z(\a[[:alnum:]-]*\)\>/ end=/<\/\z1>/ contains=TOP contained keepend transparent
